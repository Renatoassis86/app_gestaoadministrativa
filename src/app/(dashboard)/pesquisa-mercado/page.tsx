...
export default async function PesquisaMercadoPage({ searchParams }: Props) {
  ...
  const porInteresse    = count(todos, 'interesse_solucao_cve')
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <div style={{ padding: '1.5rem 2.5rem' }}>
          ...
          <div style={{ marginBottom: '2rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr 1fr', gap: '.85rem', marginBottom: '.85rem' }}>
              {[
                ...
                { label: 'Interesse Alto CVE', val: kpiLeads.interesseAlto.toLocaleString('pt-BR'), sub: 'declaram muito interesse', cor: '#d97706', bg: '#fffbeb', border: '#fde68a' },
              ].map(k => (
        ...
        {/* ── Confessionalidade + Interesse CVE ────────── */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1.25rem', marginBottom: '1.5rem' }}>
          ...
          <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 14, padding: '1.25rem', boxShadow: '0 1px 4px rgba(15,23,42,.05)' }}>
            <div style={{ fontSize: '.68rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.07em', color: '#64748b', marginBottom: '1rem', fontFamily: 'var(--font-montserrat,sans-serif)' }}>Interesse na Solução CVE</div>
          ...
          <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 14, overflow: 'hidden', boxShadow: '0 1px 4px rgba(15,23,42,.05)', marginBottom: '1.5rem' }}>
            ...
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 900 }}>
                <thead>
                  <tr style={{ background: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                    {['Escola','Cidade/UF','Tipo','NPS','Interesse CVE','Confessionalidade','Alunos','Investimento'].map(c => (
                  ...
                  {comEscola.slice(0, 50).map((r, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid #f1f5f9', background: i % 2 === 0 ? '#fff' : '#fafafa' }}>
                      ...
                      <td style={{ padding: '.65rem .85rem', fontSize: '.68rem', maxWidth: 120, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {r.interesse_solucao_cve ? <span style={{ background: '#f0fdf4', color: '#16a34a', border: '1px solid #86efac', padding: '.1rem .4rem', borderRadius: 99, fontFamily: 'var(--font-montserrat,sans-serif)', fontWeight: 600, fontSize: '.62rem' }}>{String(r.interesse_solucao_cve).slice(0, 20)}</span> : <span style={{ color: '#cbd5e1' }}>—</span>}
                      </td>
