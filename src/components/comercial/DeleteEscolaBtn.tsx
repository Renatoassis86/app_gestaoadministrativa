...
export function DeleteEscolaBtn({ escolaId, escolaNome, variant = 'sidebar', onDeleted }: Props) {
  ...
  async function handleDelete() {
    ...
    if (result.success) {
      if (onDeleted) { onDeleted(escolaId); setLoading(false); setConfirm(false); router.refresh() }
      else router.push('/comercial/escolas')
    } else {
  ...
  if (variant === 'icon') {
    return (
      <button
        ...
        disabled={loading}
        title={confirm ? 'Clique para confirmar' : `Excluir ${escolaNome}`}
        style={{
